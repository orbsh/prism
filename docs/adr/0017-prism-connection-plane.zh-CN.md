...
网关将连接持有在两个集合中——未认证集合与已认证集合。`login` 成功时把 WS 连接从未认证集合移入已认证集合。群发（向全体已认证用户广播，或向声明的子集）直接遍历目标集合，而不是每条消息对所有连接逐个按认证状态过滤。
...
- **两个连接集合表达的是一对多的成员关系，不是复制的状态事实。** 认证状态从 device↔user 绑定推导，但绑定本身是一（user）对多（device）的：一个用户的多个设备各自持有一条连接，连接集合按连接持有成员，群发按集合遍历天然覆盖"一个用户的全部在线设备"。集合成员是 per-connection 的派生视图；另一条连接上的 logout/re-login、或服务端撤销绑定，都会让缓存的集合成员关系过时。登录时的集合移动是唯一的写点；未来任何服务端撤销都必须同时遍历两个集合。接受它是因为群发遍历是热路径，逐消息认证过滤不是。
